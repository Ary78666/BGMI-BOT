...
bot = telebot.TeleBot('7468745457:AAHyZK-DcFhlZxbjkVzoIoFAE606pZgUzyM')
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in user_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 5000:
        ...
        response = "🚫 Unauthorized Access!🚫Please buy access :- @Shubhamraj90"
...
def welcome_start(message):
    ...
    response = f'''🫡WELCOME TO  :Shubhamraj server Crasher BOT 🚀 @Shubhamraj90 '''
    bot.reply_to(message, response)
...
def welcome_rules(message):
    user_name = message.from_user.first_name
    response = f'''
1. ANY PROBLEM DM HERE -> @Shubhamraj90'''
    bot.reply_to(message, response)
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 1:
            message_to_broadcast = "⚠️ THIS MESSAGE SEND BY DDOS OWNER:@Shubhamraj90\n\n" + command[1]
            with open(USER_FILE, "r") as file:
                user_ids = file.read().splitlines()
                for user_id in user_ids:
                    try:
